...
JsValidation.Form.invalidateField = function(element, errorMessages, event) {
    ...
    var decorators = _defaultDecorators,
        length,
        iteration = 0,
        options
    ;
...
JsValidation.Form.liveValidate = function(rules, options) {
    ...
    var asignLiveEvent = function(element, event, filter) {
        element.bind(
            event + '.' + filter,
            function() {
                $(this).val(JsValidation.Form.Filter[filter]($(this).val()));
            }
        );
    };
    ...
    for(var elementId in rules) {
        ...
        for (var filter in rule.filters) {
            ...
            asignLiveEvent(element, rule.filters[filter].event, rule.filters[filter].filter);
        }
...
JsValidation.Form.validate = function(form, options)
{
    ...
    var _invalid = function(element, errorMessages) {
        ...
        length = errorMessages.length;
        for (var i = 0; i < length; i++) {
            errorMessagesContainer += errorMessages[i] + '<span data-event="submit" />';
            iteration++;
