/**
 * @copyright 2012 Ricardo Buquet
 * @author Ricardo Buquet - buquet.ricardo@gmail.com
 * @website https://github.com/enkil2003/jsvalidation
 * 
...
JsValidation.Form.validateField = function(element) {
...
JsValidation.Form.liveValidate = function(rules, options) {
    ...
    var addLiveValidator = function(element, event, rules) {
        element.bind(
            ...
            function() {
                JsValidation.Form.validateField(element);
            }
        );
        ...
        element.bind(
            ...
            function() {
                ...
                for (var rule in rules) {
                    if (!rules.hasOwnProperty(rule) || rules[rule] === 'NotRequired') {
                        continue;
                    }
    ...
    for(var elementId in rules) {
        if (!rules.hasOwnProperty(elementId)) {
            continue;
        }
        ...
        for (var filter in rule.filters) {
            if (!rule.filters.hasOwnProperty(filter)) {
                continue;
            }
            addLiveFilter(element, rule.filters[filter].event || 'blur', rule.filters[filter].filter || rule.filters[filter]);
        }
        ...
        for (var validatorName in rule.validators) {
            if (!rule.validators.hasOwnProperty(validatorName)) {
                continue;
            }
            // check if there are more than one event for validation process
            if (rule.validators[validatorName].event && rule.validators[validatorName].event.indexOf(',')) {
                var events = rule.validators[validatorName].event.split(',');
                for (var i in events) {
                    if (!events.hasOwnProperty(i)) {
                        continue;
                    }
                    validatorsEvents[JsValidation.Form.Filter.StringTrim(events[i])].push(validatorName);
                }
            } else {
                validatorsEvents[rule.validators[validatorName].event || 'blur'].push(validatorName);
            }
        }
        for (var eventType in validatorsEvents) {
            if (!validatorsEvents.hasOwnProperty(eventType)) {
                continue;
            }
...
JsValidation.Form.validate = function(form, options) {
    ...
    var settings = $.extend(
        {
            ...
            valid: JsValidation.Form.validateField,
    ...
    for(var elementId in JsValidation.Forms[formId].elements) {
        if (!JsValidation.Forms[formId].elements.hasOwnProperty(elementId)) {
            continue;
        }
        ...
        for(var filterIndex in formElements[elementId].filters) {
            if (!formElements[elementId].filters.hasOwnProperty(filterIndex)) {
                continue;
            }
        ...
        for(var validatorName in formElements[elementId].validators) {
            if (!formElements[elementId].validators.hasOwnProperty(validatorName)) {
                continue;
            }
        ...
        if (errorMessages.length === 0) {
            settings.valid($(element), settings);
        } else {
...
JsValidation.Form.Validator = {
    ...
    _prepareMessageVariables: function(params) {
        ...
        for (var i = 0, length = params.messageVariables.length; i < length; i++) {
    ...
    Dependency: function(value, params) {
        ...
        if (
            ...
            for (validator in params.dependantRules) {
                if (!params.dependantRules.hasOwnProperty(validator)) {
                    continue;
                }
    ...
    GreaterThanField: function(value, params) {
        var _return = {isValid: true};
        value = parseFloat(value, 10);
        var max = parseFloat($(params.fieldSelector).val(), 10);
        if (max < value) {
            _return.isValid = false;
            _return.errorMessage = this._prepareErrorMessages(
                params.message,
    ...
    LowerThan: function(value, params) {
        var _return = {isValid: true};
